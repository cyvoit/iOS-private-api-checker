...
#### 1. 构建私有api库 ####
 ...
 - 修改config.py中sdks_config字典，增加各个version的sdk路径（目前使用的是sdk10.2，生成的db文件比较大，无法上传），然后运行build_api_db.py，会自动解析私有api，并存储到ios_private.db中。


#### 2. 检查ipa私有api ####

需要在iOS-private-api-checker文件夹下创建一个tmp文件夹。
